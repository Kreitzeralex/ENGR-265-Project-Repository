...
def calculate_stress(force, sample_diameter):
    ...
    area = (((sample_diameter) ** 2) * np.pi) / 4
...
def calculate_elastic_modulus(strain, stress):
    ...
    # find forty percent mark to subtract from all stress points and make new list
    ...
    for value in stress:
        ...
        Linear_region_stress.append(new_point)

    # find index where the minimum point is
    # this is where the stress is closest to the 40% point
...
if __name__ == "__main__":
    ...
    plt.scatter(linear_strain, linear_stress)
    plt.xlabel('Strain (%)')
    plt.ylabel('Stress (MPa)')
